...
export function selectEventsForFencer(fencer:Fencer) {
    ...
    if (fencer && is_valid(fencer.id) && is_valid(data.currentEvent.id)) {
        let weaponevents = {}; // stores the events qualified for this fencer based on weapon
        let allweaponevents = {}; // stores all events based on weapon
                ...
                if (event.competition.category && event.competition.weapon) { // should always be true
                    ...
                    if(event.isAthleteEvent) {
                        ...
                        if(event.competition.category.type == 'T') {
                            ...
                            weaponevents["T" + event.competition.weapon.abbr] = event; 
                        }
                        else {
                            weaponevents[event.competition.weapon.abbr || ''] = event;
                        }
                    }
                    if(allow_registration_lower_age && filterEventCategoryYounger(fencer, event)) {
                        ...
                        if(!allweaponevents[event.competition.weapon.abbr || '']) allweaponevents[event.competition.weapon.abbr || '']=[];
                        allweaponevents[event.competition.weapon.abbr || ''].push(event);
                    }
                }
            }
            else {
            ...
            for (let i in allweaponevents) {
                ...
                if(!weaponevents[i]) {
                    ...
                    for(var j in allweaponevents[i]) {
                        ...
                        if(event.competition && event.competition.category.value > highestcat) highestcat = event.competition.category.value;
                    }
