...
class UserProvider implements UserProviderBase
{
    ...
    /* this code comes partially from wp-includes/class-phpass.php */
    private function cryptPrivate($password, $setting)
    {
        ...
        if (!str_starts_with($setting, '$P$') && !str_starts_with($setting, '$H$') && !str_starts_with($setting, '$wp$')) {
            return $output;
        }

        // this is from wp-includes/pluggable.php:wp_check_password
        if (str_starts_with($setting, '$wp$')) {
            $password_to_verify = base64_encode(hash_hmac('sha384', $password, 'wp-sha384', true));
            $check = password_verify($password_to_verify, substr($setting, 3));
            if ($check === true) {
                $output = $setting; // return the full password hash from the database if the pass checked
            }
        }
        else if (str_starts_with($setting, '$P$') || str_starts_with($setting, '$H$')) {
