...
class Check extends Controller
{
    public function index(Request $request, $competitionId)
    {
        ...
        if (empty($model) || !isset($model->ranking) || !isset($model->competition_id) || $model->competition_id != intval($competitionId)) {
            \Log::debug("no ranking, wrong competition " . json_encode($model, JSON_PRETTY_PRINT));
            $this->authorize('not/ever');
        }
        ...
        $result = $service->handle($data, $model->ranking);
